...
/**
 * A Diameter message header. See RFC3588 section 3. After you have read that
 * understanding the class is trivial. The only fields and methods you will
 * normally use are:
 * <ul>
...
public class MessageHeader {
	...
	/**
	 * Default constructor for MessageHeader. The command flags are initialized to
	 * answer+not-proxiable+not-error+not-retransmit, also known as 0. The
	 * command_code, application_id, hop_by_hop_identifier and end_to_end_identifier
	...
	/**
	 * Prepare a response from the specified request header. The proxiable flag is
	 * copied - the other flags are cleared. The command_code, application_id,
	 * hop_by_hop_identifier, end_to_end_identifier and 'proxyable' command flag are
	 * copied. The 'request', 'error' and 'retransmit' bits are cleared.
	 */
