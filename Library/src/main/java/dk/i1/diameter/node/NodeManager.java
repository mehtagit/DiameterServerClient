...
/**
 * A Node manager. The NodeManager class manages a Node instance and keeps track
 * of connections and in-/our-going messages and their end-to-end and hop-by-hop
 * identifiers. You can build proxies, redirect agents, servers and clients on
 * top of it. NodeManager is meant to be subclassed and subclasses should
 * override handleRequest() and handleAnswer()
 *
 * <p>
 * If your needs are even simpler then have a look at {@link SimpleSyncClient}
 * and {@link dk.i1.diameter.session.SessionManager}
 * <p>
 ...
 * by putting "dk.i1.diameter.node.level = ALL" into your log.properties file
 * (or equivalent)
 *
 * <p>
 ...
 * <li>handleRequest() processes the message (this is where your code is meant
 * to be)</lI>
 * <li>handleRequest() creates an answer message. Among other things it uses
 * {@link Message#prepareAnswer(Message)}
 * <li>answer() is called</li>
 * <li>The answer is passed down to the Node instance which then sends or queues
 * the message</li>
 * </ol>
 * <p>
 ...
 * <li>The request is passed down to the Node instance which sends or queues
 * it</li>
 * <li>The sendRequest() call returns</li>
 * <li>Some time passes</li<<li>The Node instance receives the answer</li>
 * <li>The answer is passed to handleAnswer(). This is where your code is meant
 * to be)
 * </ol>
 * <p>
 ...
 * <li>The Node instance receives a message (request)</li>
 * <li>The message is passed to handleRequest()</li>
 * <li>Your implementation of handleRequest() decides to forward the
 * request</li>
 * <li>forwardRequest() is called with a state object that among other things
 * remembers the ConnectionKey and the hop-by-hop identifier</li>
 * <li>The request is passed down to the Node instance which sends or queues
 * it</li>
 ...
 * <li>The answer is passed to handleAnswer(). This is where your code is meant
 * to be)
 * <li>Your code detects that the answer must be forward back to where the
 * request came from.</lI>
...
public class NodeManager implements MessageDispatcher, ConnectionListener {
	...
	/**
	 * Constructor for NodeManager. A Node instance is constructed using the
	 * specified settings, and the internal state is initialized.
	 */
	...
	/**
	 * Constructor for NodeManager. A Node instance is constructed using the
	 * specified settings, node validator (can be null) and the internal state is
	 * initialized.
	 * 
	 * @since 0.9.4
	 */
	...
	/**
	 * Start the node manager. Starts the embedded Node. For details about
	 * {@link UnsupportedTransportProtocolException} see {@link Node#start}
	 */
	...
	/**
	 * Stop the node manager. Stops the embedded Node and call handleAnswer() with
	 * null messages for outstanding requests.
	 * 
	 * @param grace_time
	 *            Maximum time (milliseconds) to wait for connections to close
	 *            gracefully.
	...
	/**
	 * Wait until at least one connection has been established. Waits until at least
	 * one connection to a peer has been established and capability-exchange has
	 * finished.
	 * 
	 * @since 0.9.1
	 */
	...
	/**
	 * Wait until at least one connection has been established or until the timeout
	 * expires. Waits until at least one connection to a peer has been established
	...
	/**
	 * Handle a request. This method is called when a request arrives. It is meant
	 * to be overridden by a subclass. This implementation rejects all requests.
	 * <p>
	 * Please note that the handleRequest() method is called by the networking
	 * thread and messages from other peers cannot be received until the method
	 * returns. If the handleRequest() method needs to do any lengthy processing
	 * then it should implement a message queue, put the message into the queue, and
	 * return. The requests can then be processed by a worker thread pool without
	 * stalling the networking layer.
	 * 
	 * @param request
	 *            The incoming request.
	 * @param connkey
	 *            The connection from where the request came.
	 * @param peer
	 *            The peer that sent the request. This is not the originating peer
	 *            but the peer directly connected to us that sent us the request.
	 */
	protected void handleRequest(Message request, ConnectionKey connkey, Peer peer) {
		...
		System.out.println("Handling incoming request, command_code=" + request.hdr.command_code + ", peer="
				+ peer.host() + ", end2end=" + request.hdr.end_to_end_identifier + ", hopbyhop="
				+ request.hdr.hop_by_hop_identifier);
		// logger.log(Level.FINE,"Handling incoming request,
		// command_code="+request.hdr.command_code+", peer="+peer.host()+",
		// end2end="+request.hdr.end_to_end_identifier+",
		// hopbyhop="+request.hdr.hop_by_hop_identifier);
		answer.prepareResponse(request);
		answer.hdr.setError(true);
		answer.add(new AVP_Unsigned32(ProtocolConstants.DI_RESULT_CODE,
				ProtocolConstants.DIAMETER_RESULT_UNABLE_TO_DELIVER));
	...
	/**
	 * Handle an answer. This method is called when an answer arrives. It is meant
	 * to be overridden in a subclass.
	 * <p>
	 * Please note that the handleAnswer() method is called by the networking thread
	 * and messages from other peers cannot be received until the method returns. If
	 * the handleAnswer() method needs to do any lengthy processing then it should
	 * implement a message queue, put the message into the queue, and return. The
	 * answers can then be processed by a worker thread pool without stalling the
	 * networking layer.
	 * 
	 * @param answer
	 *            The answer message. Null if the connection broke.
	 * @param answer_connkey
	 *            The connection from where the answer came.
	 * @param state
	 *            The state object passed to sendRequest() or forwardRequest()
	 */
	...
	/**
	 * Answer a request. The answer is sent to the connection. If the connection has
	 * been lost in the meantime it is ignored.
	 * 
	 * @param answer
	 *            The answer message.
	 * @param connkey
	...
	/**
	 * Forward a request. Forward the request to the specified connection. The
	 * request will automatically get a route-record added if not already present.
	 ...
	 * @param state
	 *            A state object that will be passed to handleAnswer() when the
	 *            answer arrives. You should remember the ingoing connection and
	 ...
	 */
	protected final void forwardRequest(Message request, ConnectionKey connkey, Object state)
	 ...
	 */
	protected final void forwardRequest(Message request, ConnectionKey connkey, Object state, long timeout)
			throws StaleConnectionException, NotARequestException, NotProxiableException {
	...
	/**
	 * Forward an answer. Forward the answer to to the specified connection. The
	 * answer will automatically get a route-record added. This method is meant to
	 * be called from handleAnswer(). Remember to restore the hop-by-hop-identifier
	 * on the message before calling this function.
	 * 
	 * @param answer
	 ...
	 *             If the answer has the R bit set in the header.
	 * @throws NotProxiableException
	 *             If the answer does not have the P bit set in the header.
	 *             <strong>This indicates that there is something completely wrong
	 *             with either the message, the peer or your application</strong>
	 * @throws StaleConnectionException
	 *             If the ConnectionKey refers to a lost connection.
	 */
	protected final void forwardAnswer(Message answer, ConnectionKey connkey)
	...
	/**
	 * Sends a request. A request initiated by this node is sent to the specified
	 * connection. The hop-by-hop identifier of the message is set. This is not
	 * symmetric with the other sendRequest method.
	 * 
	 * @param request
	 *            The request.
	 * @param connkey
	 ...
	 *             If the ConnectionKey refers to a lost connection.
	 */
	public final void sendRequest(Message request, ConnectionKey connkey, Object state)
	 ...
	 * @since 0.9.6.8 timeout parameter introduced
	 */
	public final void sendRequest(Message request, ConnectionKey connkey, Object state, long timeout)
			throws StaleConnectionException, NotARequestException {
	...
	/**
	 * Sends a request. The request is sent to one of the peers and an optional
	 * state object is remembered. Please note that handleAnswer() for this request
	 * may get called before this method returns. This can happen if the peer is
	 * very fast and the OS thread scheduler decides to schedule the networking
	 * thread. The end-to-end identifier of the message is set. This is not
	 * symmetric with the other sendRequest method.
	 * 
	 * @param request
	 *            The request to send.
	 ...
	 *            The candidate peers
	 * @param state
	 *            A state object to be remembered. This will be passed to the
	 *            handleAnswer() method when the answer arrives.
	 * @throws NotARequestException
	 *             If the request does not have the R bit set in the header.
	 * @throws NotRoutableException
	 *             If the message could not be sent to any of the peers.
	 */
	public final void sendRequest(Message request, Peer peers[], Object state)
	 ...
	 * @since 0.9.6.8 timeout parameter introduced
	 */
	public final void sendRequest(Message request, Peer peers[], Object state, long timeout)
			throws NotRoutableException, NotARequestException {
	...
	// messagedispatcher
	/**
	 * Handle an incoming message. This implementation calls handleRequest(), or
	 * matches an answer to an outstanding request and calls handleAnswer().
	...
	public final boolean handle(Message msg, ConnectionKey connkey, Peer peer) {
		if (msg.hdr.isRequest()) {
			...
			System.out.println("Handling answer, hop_by_hop_identifier=" + msg.hdr.hop_by_hop_identifier);
	...
	// connectionlistener
	/**
	 * Handle a a connection state change. If the connection has been lost this
	 * implementation calls handleAnswer(null,...) for outstanding requests on the
	 * connection. Subclasses should not override this method.
	 */
	public final void handle(ConnectionKey connkey, Peer peer, boolean up) {
		synchronized (req_map) {
			if (up) {
				// register the new connection
				System.out.println("connkey " + connkey + ", hop_by_hop_identifier=");
				req_map.put(connkey, new HashMap<Integer, RequestData>());
			} else {
				// find outstanding requests, and call handleAnswer with NULL
				System.out.println("else connkey " + connkey + ", hop_by_hop_identifier=");
				Map<Integer, RequestData> e_c = req_map.get(connkey);
				if (e_c == null)
					return;
	...
	/**
	 * Thread for handling request timeouts. The thread is not accurate but does
	 * ensure that pending requests are timed out
	 */
