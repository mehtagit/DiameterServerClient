...
/**
 * A Diameter Message. The Message is a container for the {@link MessageHeader}
 * and the {@link AVP}s. It supports converting to/from the on-the-wire format,
 * and manipulating the AVPs. The class is lean and mean, and does as little
...
public class Message {
	...
	/**
	 * The default constructor. The header is initialized to default values and the
	 * AVP list will be empty
	 */
	public Message() {
	 ...
	 * 
	 * @param header
	 *            The message header to use instead of a default one.
	 */
	 ...
	 * 
	 * @param b
	 *            The byte array which must be large enough.
	 */
	 ...
	 *            The byte array
	 * @param offset
	 *            The offset into the byte array where the message is supposed to
	 *            start.
	...
	public enum decode_status {
		...
		decoded,
		/**
		 * The buffer appears so far to contain a valid message byte there is not enough
		 * bytes for it
		 */
		not_enough,
	...
	/**
	 * Decode a message from on-the-wire format. Implemented as return
	 * decode(b,0,b.length)
	 * 
	 * @param b
	 *            A byte array which consists of exacly 1 message. Superfluous bytes
	 *            are not permitted.
	...
	/**
	 * Decode a message from on-the-wire format. The message is checked to be in
	 * valid format and the VPs to be of the correct length etc. Invalid/reserved
	 * bits are not checked.
	 * 
	 * @param b
	 *            A byte array possibly containing a Diameter message
	 * @param offset
	 *            Offset into the array where decoding should start
	 * @param bytes
	...
	private class AVPIterator implements Iterator<AVP> {
		...
		public boolean hasNext() {
			while (i.hasNext()) {
				AVP a = i.next();
				if (a.code == code && (vendor_id == 0 || a.vendor_id == vendor_id)) {
					i.previous();
					return true;
				}
			}
	...
	public Iterator<AVP> iterator(int code) {
	...
	/**
	 * Prepare a response the the supplied request. Implemented as
	...
	/**
	 * Prepare an answer from the specified request header. This is identical to
	 * prepareResponse().
	 * 
	 * @since 0.9.3
	 */
	...
	/**
	 * Returns a iterable subset of the AVPs. This is mainly useful for the
	 * new-style foreach statement as in
	 * 
	 * <pre>
	...
	/**
	 * Finds an AVP with the specified code. Implemented as
	 * <code>find(code,0);</code>
	 */
	...
	/**
	 * Finds an AVP with the specified code/vendor-id. Returns an AVP with the
	 * specified code (and vendor-id) if any. If there are no AVPs in this message
	 * with the specified code/vendor-id then null is returned. It is unspecified
	 * which AVP is returned if there are multiple matches.
	 * 
	 * @param code
	 *            AVP code
	 * @param vendor_id
	...
	public String toString() {
		return "Message [hdr=" + hdr + ", avp=" + avp + "]";
	}
