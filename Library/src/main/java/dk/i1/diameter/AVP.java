...
/**
 * A Diameter AVP. See RFC3588 section 4 for details. An AVP consists of a code,
 * some flags, an optional vendor ID, and a payload.
 * <p>
...
public class AVP {
	...
	private int flags;
	/**
	 * The vendor ID. Assigning directly to this has the delayed effect of of
	 * setting/unsetting the vendor flag bit
	 */
	...
	/**
	 * Default constructor The code and vendor_id are initialized to 0, no flags are
	 * set, and the payload is null.
	 */
	public AVP() {
	}
	...
	/**
	 * Returns the payload of the AVP Returns a copy of the (unpadded) payload of
	 * the AVP in network byte order. This is normally not what you want. The
	 * subclasses have much nicer interfaces.
	 * 
	 * @since 0.9.6.5
	 */
	...
	/**
	 * Sets the M-bit and returns the instance. The M-bit (mandatory) is set and the
	 * instance is returned. While this is very very simple, it can be useful when
	 * constructing grouped AVPs and you need to set the M-bit on embedded AVPs, as
