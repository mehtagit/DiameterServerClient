...
import com.nmss.messages.*;
import com.nmss.pojo.*;
...
public class Client {
    ...
    private void disConnect() {
        try {
            ...
            System.out.println("Disconnected from server " + this);
        } catch (Exception e) {
    ...
    private boolean connect() {
        ...
        try {
            ...
            System.out.println("Connection Created " + this);
            isConnected = true;
        } catch (Exception e) {
            logger.error(this + e.getMessage(), e);
            isConnected = false;
    ...
    public void monitor() {
        ...
        if (!isMonitorThreadRunning.get()) {
            ...
            System.out.println(this + " trying to make connection");
            try {
                while (!connect()) {
                ...
                while (!isCerSuccess) {
                    ...
                    System.out.println("CER : " + cerMessage);
                    Message cea = readMessage();
                    System.out.println("CEA : " + cea);
                    try {
    ...
    private void dwrTask() {
        ...
        while (true) {
            ...
            if (isMonitorThreadRunning.get()) {
                System.out.println("DWR Thread Stopped " + this);
                break;
            }
            ...
            try {
                if (((System.currentTimeMillis() - lastRequestSend) / 1000) > 10) {
                    ...
                    System.out.println("Send DWR : " + dwrMessage);
    ...
    private void writeInToServerStream() {
        ...
        while (true) {
            ...
            try {
                ...
                System.out.println("Request Received at DC" + requestData + ", Queue:" + requestQueue.size());
                ...
                switch (requestData.getRequestType()) {
                    ...
                    case ECR:
                        message = new ECRMessage(diameterServer.getOriginIp(), diameterServer.getOriginRelam(),
                                diameterServer.getDestinationIP(), diameterServer.getDestinationRelam(),
                                diameterServer.getVendorId(), diameterServer.getAuthApp(), requestData.getImpi(),
                                requestData.getTid(), isProxy,
                                requestData.getMar() == null ? null : requestData.getMar().getAuthenticationScheme(),
                ...
                System.out.println("MAR : " + message);

                /* for DWR */
                this.lastRequestSend = System.currentTimeMillis();
                window.incrementAndGet();
                System.out.println("window " + window.get());
    ...
    private void readFromServerStream() {
        ...
        while (true) {
            try {
                if (isMonitorThreadRunning.get()) {
                    System.out.println("Receiver Thread From Server to Client Stopped : " + this);
                    break;
                }
                ...
                if (response != null) {
                    ...
                    switch (response.hdr.command_code) {
                        case ProtocolConstants.DIAMETER_MULTIMEDIA_AUTHENTICATION_REQUEST:
                            ...
                            System.out.println("MAA : " + responseData + ", Queue:" + responseQueue.size());
                        ...
                        case ProtocolConstants.DIAMETER_USER_DATA_REQUEST:
                            ...
                            System.out.println("UDA : " + responseData + ", Queue:" + responseQueue.size());
                            putResponseBackInQueue(responseData);
                        ...
                        case ProtocolConstants.DIAMETER_COMMAND_DEVICE_WATCHDOG: // DWA
                            ...
                            if (resultCode == 2001) {
                                ...
                                System.out.println("Send DPR " + dprMessage);
                                sendMessage(dprMessage);
                            }

                            break;
                        case ProtocolConstants.DIAMETER_ME_IDENTITY_CHECK_REQUEST:
                            resultCode = new AVP_Unsigned32(response.find(ProtocolConstants.DI_RESULT_CODE))
                                    .queryValue();
                            responseData = parseECA(response, resultCode);
                            System.out.println("ECA : " + responseData + ", Queue:" + responseQueue.size());
                            putResponseBackInQueue(responseData);
    ...
    private DiameterData parseECA(Message response, int resultCode) {
        DiameterData responseData = new DiameterData();
        responseData.setEca(new ECA());

        responseData.setImpi(new AVP_UTF8String(response.find(1)).queryValue());
        responseData.setTid(new AVP_UTF8String(response.find(ProtocolConstants.DI_SESSION_ID)).queryValue());
        responseData.setResult(resultCode);
        if (resultCode == RESULT_CODE_STATUS.DIAMETER_SUCCESS) {
            // 612 = group
            try {
                AVP_Unsigned32 equipmentStatusAVP = new AVP_Unsigned32(response.find(ProtocolConstants.DI_EQUIPMENT_STATUS));
                if (equipmentStatusAVP != null) {
                    int value = new AVP_Unsigned32(response.find(ProtocolConstants.DI_EQUIPMENT_STATUS))
                            .queryValue();
                    responseData.getEca().setStatus(ECR_RESULT.getValue(value));
                }
            } catch (Exception e) {
                logger.error(e.getMessage(), e);
                responseData.getEca().setStatus(ECR_RESULT.NA);
            }
        }
