...
class TestSessionTest {
	...
	public static final void main1(String args[]) throws Exception {
		...
		try {
			node_settings = new NodeSettings("TestSessionTest.example.net", "example.net", 99999, // vendor-id
					capability, 9999, // 3868, //port must be non-zero because we have sessions
		...
		Peer peers[] = new Peer[] { new Peer(args[0]) };
