...
class cc_test_server extends NodeManager {
	...
	public static final void main(String args[]) throws Exception {
		/*
		 * if (args.length < 2) {
		 * System.out.println("Usage: <host-id> <realm> [<port>]"); return; }
		 */
		String host_id = "103.206.248.235";
		String realm = "90@abc..com";
		int port;
		if (args.length >= 3)
		...
		System.out.println("NodeSettings : " + node_settings);
	...
	protected void handleRequest(dk.i1.diameter.Message request, ConnectionKey connkey, Peer peer) {
		...
		System.out.println("handleRequest: cc_test_server :: Request Received : " + request);
		Message answer = new Message();
		answer.prepareResponse(request);
		System.out.println("Answer Prepared : " + answer);
		AVP avp;
		...
		if (avp == null) {
			...
			System.out.println("DIAMETER_RESULT_MISSING_AVP Returned with added new AVP : " + avp);
			return;
		}
		...
		System.out.println("cc_request_type : " + cc_request_type);
		if (cc_request_type != ProtocolConstants.DI_CC_REQUEST_TYPE_INITIAL_REQUEST
			...
			System.out.println(" DIAMETER_RESULT_INVALID_AVP_VALUE Returned with added new AVP : " + avp);
			return;
		}
		...
		if (avp != null) {
			answerError(answer, connkey, ProtocolConstants.DIAMETER_RESULT_INVALID_AVP_VALUE,
					new AVP[] { new AVP_Grouped(ProtocolConstants.DI_FAILED_AVP, new AVP[] { avp }) });
			System.out.println("DI_MULTIPLE_SERVICES_CREDIT_CONTROL Returned with added new AVP : " + avp);
			return;
		}
		...
		if (avp != null) {
			...
			if (indicator != ProtocolConstants.DI_MULTIPLE_SERVICES_INDICATOR_MULTIPLE_SERVICES_NOT_SUPPORTED) {
				answerError(answer, connkey, ProtocolConstants.DIAMETER_RESULT_INVALID_AVP_VALUE,
						new AVP[] { new AVP_Grouped(ProtocolConstants.DI_FAILED_AVP, new AVP[] { avp }) });
				System.out.println("DI_MULTIPLE_SERVICES_INDICATOR Returned with added new AVP : " + avp);
				return;
			}
		}
		...
		if (avp != null)
			answer.add(avp);
		System.out.println("Added new AVP : " + avp);
		...
		case ProtocolConstants.DI_CC_REQUEST_TYPE_TERMINATION_REQUEST:
			...
			System.out.println("Switch Case DI_GRANTED_SERVICE_UNIT Added new AVP : " + avp);
			break;
		case ProtocolConstants.DI_CC_REQUEST_TYPE_EVENT_REQUEST: {
			...
			if (avp == null) {
				...
				System.out.println("Switch Case DI_CC_REQUEST_TYPE_EVENT_REQUEST Added new AVP : " + avp);
				return;
			}
			...
			case ProtocolConstants.DI_REQUESTED_ACTION_DIRECT_DEBITING:
				...
				System.out.println("Switch Case DI_REQUESTED_ACTION_DIRECT_DEBITING ");
				break;
			case ProtocolConstants.DI_REQUESTED_ACTION_REFUND_ACCOUNT:
				// nothing. just indicate success
				System.out.println("Switch Case DI_REQUESTED_ACTION_REFUND_ACCOUNT ");
				break;
			case ProtocolConstants.DI_REQUESTED_ACTION_CHECK_BALANCE:
				...
				System.out.println("Switch Case DI_REQUESTED_ACTION_CHECK_BALANCE Added new AVP : " + avp);
				break;
			case ProtocolConstants.DI_REQUESTED_ACTION_PRICE_ENQUIRY:
				...
				answer.add(new AVP_Grouped(ProtocolConstants.DI_COST_INFORMATION,
						new AVP[] {
								new AVP_Grouped(ProtocolConstants.DI_UNIT_VALUE,
				...
				System.out.println("Switch Case DI_REQUESTED_ACTION_PRICE_ENQUIRY Added new AVP : " + avp);
				break;
			default: {
				answerError(answer, connkey, ProtocolConstants.DIAMETER_RESULT_INVALID_AVP_VALUE,
						new AVP[] { new AVP_Grouped(ProtocolConstants.DI_FAILED_AVP, new AVP[] { avp }) });
				System.out.println("Switch Case Default Added new AVP : " + avp);
				return;
			}
			}
		}
